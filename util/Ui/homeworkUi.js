...
export default function HomeworkUi(props) {
  ...
  useEffect(() => {
    ...
    const userDataHomeWork = async () => {
      const userHomeWork = await getData(
        `/gethomeworkstudent/${isAuthenticated().user._id}/${props.homeworkID}`,
  ...
  const handleInput = (user) => async (el) => {
    ...
    let data = await postDataForm(
      `/addhomeworkbylectureid/${props.id}`,
      bodyFormData,
      isAuthenticated().token
    );
...
{console.log(props.status)}
  ...
  <path d="M5 11v-6a2 2 0 0 1 2 -2h7l5 5v11a2 2 0 0 1 -2 2h-9.5" />
      ...
      {/* <div className="text-[#606F7B]   border-b-[rgba(246,247,249,.05)]  border-b-[1px] flex flex-col flex-wrap  m-0 p-0 justify-center items-center gap-1 ">
        {userHomeWorkData &&
        ...
        <div>
          ...
          <div className="">
            <div>
              {" "}
              <p className=" font-normal p-0 m-0 text-[0.9rem]">
                {props.lname}
              </p>
            </div>
            <div></div>
      ...
      <table>
        ...
        <tr>
          ...
          {moment(props.uploaded).format("MMMM Do YYYY")}
          </td>
          <td>
         { moment(props.lastupload).format("MMMM Do YYYY") }
          </td>
          <td>
          <tr className="">
            {" "}
