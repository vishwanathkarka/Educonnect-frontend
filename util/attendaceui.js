...
<Image src={props.img?props.img:"https://res.cloudinary.com/dfceagnv7/image/upload/v1669976521/users/nlqbwvkoj7chzzdwm3p9.jpg"} alt="" width={500}
  ...
  {props.id?  <div className='flex justify-around gap-3 items-center  text-center'>
   <input type="radio" name={props.name} value="Regular" onClick={()=>props.attendnceData({"userId":props.id,"isPresent":true,"date":props.date,"lectureId":isAuthenticated().user._id})}  />
      <label htmlFor="present" >Present</label>
      <input type="radio" name={props.name} value="Medium" onClick={()=>props.attendnceData({"userId":props.id,"isPresent":false,"date":props.date,"lectureId":isAuthenticated().user._id})}   />
