...
<div className=" m-0">
{departments && departments.map((dep,index)=>(<> <ul><li  className="text-white m-0 flex gap-3 items-center"> <p>{dep.department.department}</p>
<svg xmlns="http://www.w3.org/2000/svg" className="icon icon-tabler icon-tabler-pencil cursor-pointer" width="20" height="20" viewBox="0 0 24 24" stroke-width="1.5" stroke="#ffffff" fill="none" stroke-linecap="round" stroke-linejoin="round" onClick={()=>{setViewDep(!viewDep)}}>
...
<ul>
   { dep.section.map(sec=>( <li className="flex gap-3"><p>{sec.section}</p>
   <svg xmlns="http://www.w3.org/2000/svg" className="icon icon-tabler icon-tabler-pencil cursor-pointer" width="20" height="20" viewBox="0 0 24 24" stroke-width="1.5" stroke="#ffffff" fill="none" stroke-linecap="round" stroke-linejoin="round" onClick={()=>{setViewDep(!viewDep)}}>
  <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
  <path d="M4 20h4l10.5 -10.5a1.5 1.5 0 0 0 -4 -4l-10.5 10.5v4" />
  <line x1="13.5" y1="6.5" x2="17.5" y2="10.5" />
...
<svg xmlns="http://www.w3.org/2000/svg" className="icon icon-tabler icon-tabler-trash cursor-pointer" width="20" height="20" viewBox="0 0 24 24" stroke-width="1.5" stroke="#ff4500" fill="none" stroke-linecap="round" stroke-linejoin="round">
  <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
  <line x1="4" y1="7" x2="20" y2="7" />
  <line x1="10" y1="11" x2="10" y2="17" />
  <line x1="14" y1="11" x2="14" y2="17" />
...
{viewDep && <select
            className="block w-full rounded-md border-0 py-2 pl-7 pr-20 text-gray-900 text-lightwg ring-1 bg-secoundblack ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6 my-2"
            id="role"
          ...
          >
            ...
            {/* <option value="student">student</option>
            <option value="parent">parent</option> */}
           {departments.map((dep,index)=>( <option value={dep.department._id} key={dep._id}>{dep.department.department}</option> ))  }
          </select>
}
          <button
            type="submit"
   ...
   <div className="flex gap-3 px-3 bg-black ">
       ...
       <select
          ...
          <select className="block w-full rounded-md border-0 bg-secoundblack text-white py-1.5 pl-7 pr-20 text-gray-900 ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6 my-4" id = "role"
            onChange={handleInput("role")} >
          <option selected >Choose Role ...</option>
            <option value="student" key="student" selected={activeSelect("student",router.query.role)}  key="1">student</option>
...
<div className="flex">
  { userInfo && userInfo.user.map(user => (<> <AdminCard key={user._id} photo ={user.photo.secure_url} name ={user.firstName}  closeForm = {formViewStatus}  />  {viewForm && (<FormPer  closeForm = {formViewStatus} name={user.firstName} email = {user.email} role = {user.role} parentEmail={user.parentEmail} htno = {user.htno} phoneNo={user.phoneNo} departments = {user.departments} />)} </>))}
  </div>
  </div> </>
  )
}
